...
它解决的问题并不是 “状态管理。”，这种说法是非常模糊的。React 已经做了状态管理。Redux 辅助状态管理，但这并不是它解决的问题。
...
你可以将数据想象成是电流，组件通过电线跟数据联系在一起。数据在这些电线中进行上下传递，但电线又不能凭空运行 —— 它们需要跟组件进行关联。
...
你迟早会遇到这样一种情况：顶级容器组件中的一些数据需要被其下 4 层的子组件所用到。这里有一个来自 Twitter 的截图，所有头像都标注出来了：
...
更重要的是，这并不是一个好的软件设计。那些传递数据的中间组件不得不接收和传递那些它们并不需要关心的 prop。这也意味着对这些组件的重构和重用都会变得困难。
...
是的。它还有一些其它很酷的特性，如让调试变得更简单（Redux DevTools 让你能够检查每次单个 state 发生改变），状态回滚（你可以将状态回滚以便查看当时的应用），并且长期来说你还可以提升代码的可维护性。它还能够教会你很多关于函数式编程方面的知识。
...
但应该以 “将任意数据插入到任意组件” 作为事件主导。如果你不需要它，你可能根本用不着 Redux。
...
组件自身并不知道 Redux —— 它只管接收一个叫 `user` 的 prop 并把用户头像渲染出来。`mapStateToProps` 函数从 Redux store 中提取出 `user` 并映射为 `user` prop。最终，实际上就是 `connect` 函数通过 `mapStateToProps` 将数据从 Redux 提供给了 `Avatar`。
...
你也许注意到了在最后有两个 `exports` —— 一个有名字，一个是 default。这并非必须，但是对于可能会用到源组件和源组件被 Redux 包装后的版本会比较有用。
...
如果你需要在多个视图之间缓存数据 —— 例如，在用户点击详情的时候加载并缓存数据以便下一次访问更快 —— 请考虑将数据存放在 Redux 中。
...
如果你的应用变得越来越大，维护大量数据的时候，不管这之间是否有关联 —— 请考虑使用 Redux。但是一开始应该考虑不用它的情况，在你觉得它会对你有帮助的时候将它添加进你的项目里面。
...
阅读 [本系列第二章](https://daveceddia.com/how-does-redux-work)，我们将深入 Redux：如何配置它，如何将重要的部分组合起来（action、reducer 和 store）。
