...
（总结： tapable 是一个流程工具，它提供了各种钩子类，通过在特定的钩子实例上执行 `tap` 函数可以向其中添加回调函数，然后你需要在特定的生命周期调用钩子实例的 `call` 方法来执行所有通过 `tap` 添加到它上面的函数。更通俗的解释就是，tapable 就是一个用于缓存回调函数的队列，只是提供了各种执行方式 —— 如按序执行、并行、同步异步以及截断等）
