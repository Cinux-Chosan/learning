...
- `:cd {目录名}`： 将目录设置为工作目录和共享目录。
- `:pwd`： 获取当前工作目录
- `:lcd`： 记录工作目录，其它窗口的 `:cd`、`:lcd` 不会影响到该目录。

- `gf`： 将光标移动到某个字符串上，然后输入该命令，Vim 就会找到并编辑该文件。

   如果这个文件并不在工作目录中，此时 Vim 会在 path 选项中定义的目录中去查找。该选项是一个以逗号分隔的目录名列表。
   
   假设头文件都位于 "c:/prog/include"，则可以设置 path 指向这个目录：
   
   `:set path+=c:/prog/include`，相对目录也可以，如 `:set path+=./proto`，如果没有指定`./`，则在当前工作目录中查找名为 'proto' 的子目录。
   
   'isfname'选项用于告诉Vim哪些字符可以作为文件名的一部分, 哪些
不行(比如上例中的"字符).

   `gf` 命令除了会搜索 'path' 路径之外，与直接使用 `:edit` 命令没有区别。

- `CTRL-W f`：在另一个窗口打开指定文件，或者使用 `:sfind` 来代替 `:find` 命令。
