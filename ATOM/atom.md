...
你还可以通过快捷键 `Ctrl+\` 或者命令 `tree-view:toggle` 来隐藏或显示树状视图。 `Alt+\` 将会将焦点聚焦到树状视图上。当焦点在树状视图的时候，你可以通过 `A`，`D`，`M`，'Delete' 来添加、复制、移动（也可以修改文件名，同Linux mv）、删除文件或文件夹（`A`是添加文件，`shift+A` 添加文件夹）。你还可以通过在树状视图的每一项上面点击鼠标右键来查看可以操作的选项。
...
该映射定义了两种不同的上下文中关于 `Enter` 键的意义。在正常编辑的时候，按 `Enter` 键触发 `editor:newline` 命令，它将会让编辑器插入新的一行。但是如果在选择列表的迷你编辑器中敲击了同样的键，它将基于更加详细的选择器触发 `core:confirm` 命令。
