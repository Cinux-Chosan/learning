...
### 6.3.6 寄生组合式继承
      ...
      个人总结：寄生组合式继承的做法实际上就是用超类的 prototype 对象的一个副本作为子类的 prototype，然后将 constructor 指回子类构造函数。与组合继承的区别在于组合继承是使用超类的实例来作为子类的 prototype 对象，而寄生组合式继承是使用超类的 prototype 副本作为子类的 prototype 对象。
