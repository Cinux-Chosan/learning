...
函数的 `arguments` 是一个类数组对象. 数组对象包含一个非同寻常的特性:
在非严格模式下, 当一个函数包含若干形参, 实参对象的数组元素是函数形参所对应实参的别名, 实参对象中以数字索引, 并且形参名称可以认为是相同变量的不同命名. 通过实参名字来修改实参值的话, 通过 `arguments[]` 数组也可以获取到更改后的值:

```js
// 即 arguments 数组对象中包含的参数和函数参数相互绑定, 修改任一个都会反映到与之绑定的值上
function f(x) {
  console.log(x);       // 输出初始值
  arguments[0] = null;  // 修改实参数组元素同样会修改 x 的值
  console.log(x);       // 输出 null
}
```

ECMAScript 5 中移除了实参对象的这个特殊特性( 在Chrome 中测试并未移除, 可能是兼容 ECMAScript 3). 在严格模式下还有一点(与非严格模式相比)不同, 非严格模式中, 函数里的 arguments 仅仅是一个标识符, 在严格模式中, 他变成了一个保留字. 严格模式中的函数无法使用 arguments 作为形参名或局部变量名, 也不能给 arguments 赋值.

#### callee 和 caller

实参对象 arguments 还定义了 callee 和 caller 属性, **ECMAScript 5 严格模式中对这两个属性的读写操作都会导致类型错误**. 而在非严格模式下, ECMAScript 标准规范规定 callee 属性指代当前正在执行的函数. caller 是非标准的, 但大多数浏览器都实现了这个属性, 它指代调用当前正在执行的函数的函数. 通过 caller 属性可以访问调用栈.