...
本文概述了在 TypeScript 中使用名字空间(以前的"内部模块")的各种方法. 正如我们在术语中提到的, "内部模块" 就是现在的 "名字空间". 此外, 在任何使用 `module` 关键字定义内部模块的地方, 都应该使用 `namespace` 关键字代替. 这能够避免相似的命名术语使得新用户产生混淆.
...
从本页的第一个例子开始. 我们写了一些列简单的字符串验证器, 这些验证器可以用来验证用户的输入或者检查外部提供的数据文件格式.
...
我们添加越多的验证器, 就越希望能有一种组织方式能让我们跟踪我们的代码并且可以避免和其它对象产生命名冲突. 我们将自己的代码封装到一个单独的名字空间来取代把一大堆不同的声明直接放到全局名字空间中.
...
这个例子中, 我们会将所有验证相关的实体放到一个名叫 `Validation` 的名字空间中. 因为我们希望这些接口和类能在名字空间之外访问, 因此还需要使用 `export` 来导出. 相反, `lettersRegexp` 和 `numberRegexp` 变量是实现细节, 因此不导出它们, 这样在名字空间之外就无法直接访问到它们了. 在文件底部的测试代码中, 我们需要在名字空间之外使用这些类型, 如 `Validation.LettersOnlyValidator`.
...
这里, 我们将 `Validation` 名字空间切分到多个文件中. 尽管每个文件是独立的, 但是它们都服务于同一个名字空间, 并且可以被使用, 就像他们是在同一个地方定义的一样. 因为文件之间相互依赖, 我们需要添加引用标记(引用标记如 `/// <reference path="Validation.ts" />`)来告诉编译器这些文件之间的关系. 我们的测试代码保持不变.
...
一旦涉及到多个文件，我们就需要确保所有编译后的代码都被加载。有两种方法可以做到这一点。
...
第一种, 我们可以使用级联输出标记 `--outFile` 来告诉编译器将所有输入的文件全部都输出到一个文件中:
